...
def main():
    ...
    cnn_model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=args.learning_rate),
                        loss=tf.keras.losses.SparseCategoricalCrossentropy(),
                        metrics=['accuracy'])
    try:
        os.makedirs('./trained_cnn_models/%s'%args.dir_name)
    except:
        print("Directory name %s"%args.dir_name, 'exist')

    checkpoint_cb = tf.keras.callbacks.ModelCheckpoint("./trained_cnn_models/%s/%s.h5"%(args.dir_name, args.fine_trained_model_name), save_best_only=True)
    checkpoint_cb2 = tf.keras.callbacks.ModelCheckpoint("./trained_cnn_models/%s/%s_weight.h5"%(args.dir_name, args.fine_trained_model_name), save_weight_only=True)
    early_stopping_cb = tf.keras.callbacks.EarlyStopping(patience=20,
                                                    restore_best_weights=True)
    reduce_lr_plateu_cb = tf.keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=10)

    with tf.device('/CPU:0'):
        history_2 = cnn_model.fit(train_patches_scaled, train_labels_,
    ...
    plt.tight_layout()
    plt.savefig('./loss_curve/%s'%args.fine_trained_model_name)

    non_ls_patches_scaled = multi_data_scale.multi_scale(non_ls_patches_features)
    full_result = cnn_model.predict(non_ls_patches_scaled)
    ...
    plt.ylabel('Northing')
    plt.tight_layout()
    plt.savefig('./images/landslide_maps/%s.png'%args.fine_trained_model_name)
    ...
    with tf.device('/CPU:0'):
        test_results = cnn_model.predict(test_patches_scaled)
