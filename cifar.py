...
def main():
    ...
    def cifar_model(input_shape, base_model, training = False):
        ...
        return tf.keras.Model(inputs, x, name="Cifar_model")

    cifar = cifar_model(input_shape=input_shape, base_model=base_model, training=False)

    try:
        os.makedirs('./trained_cifar/%s'%args.dir_name)
    except:
        print("Directory name %s"%args.dir_name, 'exist')

    cifar.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=args.learning_rate),
                loss=tf.keras.losses.SparseCategoricalCrossentropy(),
                metrics=['accuracy'])

    checkpoint_cb = tf.keras.callbacks.ModelCheckpoint("./trained_cifar/%s/%s.h5"%(args.dir_name, args.fine_trained_model_name), save_best_only=True)
    checkpoint_cb2 = tf.keras.callbacks.ModelCheckpoint("./trained_cifar/%s/%s_weight.h5"%(args.dir_name, args.fine_trained_model_name), save_weight_only=True)
    early_stopping_cb = tf.keras.callbacks.EarlyStopping(patience=20,
                                                    restore_best_weights=True)
    reduce_lr_plateu_cb = tf.keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=10)

    with tf.device('/CPU:0'):
        history_2 = cifar.fit(train_patches_scaled, train_labels_,
    ...
    full_result = cifar.predict(non_ls_patches_scaled)
    ...
    try:
        os.makedirs('./landslide_maps/%s'%args.dir_name)
    except:
        pass
    ...
    plt.ylabel('Northing')
    plt.tight_layout()
    plt.savefig('./landslide_maps/%s/%s.png'%(args.dir_name, args.fine_trained_model_name))
    ...
    with tf.device('/CPU:0'):
        test_results = cifar.predict(test_patches_scaled)
