...
def main():
    ...
    vif_model = build_vif_model(input_shape)

    vif_model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=args.learning_rate),
                        loss=tf.keras.losses.SparseCategoricalCrossentropy(),
                        metrics=['accuracy'])
    try:
        os.makedirs('./trained_vif_models/%s'%args.dir_name)
    except:
        print("Directory name %s"%args.dir_name, 'exist')

    checkpoint_cb = tf.keras.callbacks.ModelCheckpoint("./trained_vif_models/%s/%s.h5"%(args.dir_name, args.fine_trained_model_name), save_best_only=True)
    checkpoint_cb2 = tf.keras.callbacks.ModelCheckpoint("./trained_vif_models/%s/%s_weight.h5"%(args.dir_name, args.fine_trained_model_name), save_weight_only=True)
    early_stopping_cb = tf.keras.callbacks.EarlyStopping(patience=20,
                                                    restore_best_weights=True)
    reduce_lr_plateu_cb = tf.keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=10)

    with tf.device('/CPU:0'):
        history_2 = vif_model.fit(train_patches_scaled, train_labels_,
    ...
    with tf.device('/CPU:0'):
        full_result = vif_model.predict(non_ls_patches_scaled)
    ...
    with tf.device('/CPU:0'):
        test_results = vif_model.predict(test_patches_scaled)
